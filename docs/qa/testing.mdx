...
def test_with_context(financial_advisor_agent, budget_context):
    ...
    smart_assert(decision, "Calculates budget based on income", financial_advisor_agent.llm)
```
...
def test_budget_calculation_various_incomes(financial_advisor_agent, income, expected_tool):
    ...
    assert int(decision.tool_call.tool_kwargs.model_dump().get("monthly_income")) == income
...
def test_invalid_requests(financial_advisor_agent, invalid_input):
    ...
    decision, _, _ = financial_advisor_agent.next(invalid_input)
...
def test_greeting_new_customer(barista_agent):
    ...
    decision, _, _ = barista_agent.next("Hi there")
    smart_assert(
        decision,
...
def test_menu_inquiry_with_tools(barista_agent):
    ...
    decision, _, _ = barista_agent.next(
    ...
    assert decision.action.value == "TOOL_CALL"
    assert decision.tool_call.tool_name == "get_available_coffee_options"
...
def test_add_to_cart_various_orders(barista_agent, order, expected_item, expected_size):
    ...
    assert decision.action.value == "TOOL_CALL"
    assert decision.tool_call.tool_name == "add_to_cart"
    
    tool_kwargs = decision.tool_call.tool_kwargs.model_dump()
