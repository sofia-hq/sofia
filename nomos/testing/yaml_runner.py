...
def _create_test_file(yaml_path: Path, tmpdir: Path) -> Path:
    ...
    lines: List[str] = [
        "import pytest",
        "from pathlib import Path",
        "from nomos import AgentConfig, Agent",
        "from nomos.testing import smart_assert, SessionContext",
        ...
        "        decision, _, _ = agent.next(tc.input, session_data=ctx.model_dump(mode='json'), verbose=tc.verbose)",
        "        if tc.invalid:",
