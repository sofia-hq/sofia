...
class Flow:
    ...
    def __init__(
        ...
        for component_name, component_config in config.components.items():
            if component_name in component_registry:
                self.components[component_name] = component_registry[component_name](**component_config)
    
    def _get_default_component_registry(self) -> Dict[str, Callable[..., FlowComponent]]:
    ...
    def enter(
        ...
        if not self.can_enter(entry_step):
            raise ValueError(f"Step '{entry_step}' is not a valid entry point for flow '{self.flow_id}'")
    ...
    def exit(
        ...
        if not self.can_exit(exit_step):
            raise ValueError(f"Step '{exit_step}' is not a valid exit point for flow '{self.flow_id}'")
