...
async def websocket_endpoint(websocket: WebSocket, session_id: str, initiate: bool = False, verbose: bool = False):
    ...
    try:
        ...
        if initiate:
            ...
            decision, tool_output = session.next(None, return_tool=verbose, return_step_transition=verbose)
            await session_store.set(session_id, session)
            await websocket.send_json({"message": decision.model_dump(mode="json"), "tool_output": tool_output})
            ...
            try:
                ...
                if "ping" in data:
                    ...
                    if connection_open:
                        try:
                            ...
                            print(f"Error sending pong: {type(pong_error).__name__} - {str(pong_error)}")
                            connection_open = False
                    continue
                    
                if "message" in data and data["message"]:
                    user_message = data["message"]
                    print(f"Processing user message for session {session_id}: {user_message}")
                    decision, tool_output = session.next(user_message, return_tool=verbose, return_step_transition=verbose)
                    action_type_map = {
                    ...
                    if connection_open:
                        try:
                            print(f"Sending response of type {action_type} for session {session_id}")
                            ...
                            print(f"Error sending response: {type(send_error).__name__} - {str(send_error)}")
                            ...
                            if connection_open:
                                try:
                                    ...
                                    print(f"Failed to send error message, connection may be broken")
                else:
                    # If message is missing or empty, send back an error
                    print(f"Received invalid message format for session {session_id}")
                    if connection_open:
                        await websocket.send_json({"error": "Invalid message format. Expected 'message' field with non-empty content."})
            except starlette.websockets.WebSocketDisconnect as ws_disconnect:
                # Client disconnected, exit gracefully
                print(f"Client disconnected from WebSocket for session {session_id}. Code: {ws_disconnect.code}")
                connection_open = False
                break
            except Exception as inner_e:
                # Handle any errors within the message processing loop
                print(f"Error processing message for session {session_id}: {type(inner_e).__name__} - {str(inner_e)}")
                ...
                if connection_open:
                    try:
                        ...
                        print(f"Failed to send error message, closing connection for session {session_id}")
                        connection_open = False
                        break
    except starlette.websockets.WebSocketDisconnect as ws_disconnect:
        # Handle websocket disconnection at the outer level
        print(f"WebSocket disconnected for session {session_id}. Code: {ws_disconnect.code}")
        connection_open = False
    except Exception as e:
        print(f"WebSocket endpoint error for session {session_id}: {type(e).__name__} - {str(e)}")
